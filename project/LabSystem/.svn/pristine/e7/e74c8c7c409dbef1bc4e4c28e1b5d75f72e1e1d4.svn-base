function FileProgress(file, targetID) {
	...
	if (!this.fileProgressWrapper) {
		...
		progressText.className = "image_pic";
...
FileProgress.prototype.setComplete = function(file, data) {
	this.fileProgressElement.className = "progressContainer";
	var s = eval('(' + data + ')');
	this.fileProgressElement.childNodes[1].style.backgroundImage = "url("
			+ s.data['path'] + ")";
	var textarea = document.createElement("textarea");
	textarea.name = "desc[]";
	textarea.value = file.name;
	textarea.className = "image_desc";
	var spath = document.createElement("input");
	spath.name = 'path[]';
	spath.value = s.data['path'];
	spath.type = 'hidden';
	this.fileProgressElement.appendChild(textarea);
	this.fileProgressElement.appendChild(spath);
	this.fileProgressElement
			.removeChild(this.fileProgressElement.childNodes[2]);
